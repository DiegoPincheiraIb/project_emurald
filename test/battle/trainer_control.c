...
TEST("CreateNPCTrainerPartyForTrainer generates customized Pokémon")
{
    ...
    u32 currTrainer = 0;
    u8 nickBuffer[20];
    CreateNPCTrainerPartyFromTrainer(testParty, currTrainer, &sTestTrainers[GetTrainerDifficultyLevelTest(currTrainer)][currTrainer], TRUE, BATTLE_TYPE_TRAINER);
...
TEST("CreateNPCTrainerPartyForTrainer generates different personalities for different mons")
{
    ...
    struct Pokemon *testParty = Alloc(6 * sizeof(struct Pokemon));
    CreateNPCTrainerPartyFromTrainer(testParty, 0, &sTestTrainers[difficulty][0], TRUE, BATTLE_TYPE_TRAINER);
...
TEST("Trainer Class Balls apply to the entire party")
{
    ...
    u32 j;
    struct Pokemon *testParty = Alloc(6 * sizeof(struct Pokemon));
    CreateNPCTrainerPartyFromTrainer(testParty, TRAINER_NONE, &sTestTrainer2, TRUE, BATTLE_TYPE_TRAINER);
    for(j = 0; j < 6; j++)
    {
...
TEST("Trainer Party Pool picks according to custom rules")
{
    struct Pokemon *testParty = Alloc(6 * sizeof(struct Pokemon));
    u32 currTrainer = 5;
    CreateNPCTrainerPartyFromTrainer(testParty, currTrainer, &sTestTrainers[GetTrainerDifficultyLevelTest(currTrainer)][currTrainer], TRUE, BATTLE_TYPE_TRAINER | BATTLE_TYPE_DOUBLE);
